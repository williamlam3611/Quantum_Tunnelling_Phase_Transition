...
is_folder = 0;
if (!exists("data_folder_1"))               data_folder_1       = GPVAL_PWD;
if (!exists("data_folder_2"))               data_folder_2       = GPVAL_PWD;
if (strstrt(substr(data_folder_1, 2, strlen(data_folder_1)), ".") == 0) is_folder = 1;
if (strstrt(substr(data_folder_2, 2, strlen(data_folder_2)), ".") == 0) is_folder = 1;
...
if (is_folder) {
    pwd = GPVAL_PWD;
    cd data_folder_1;
    plot for [data in data_files_1] data using (column(5)):1:(rgb($2, $3, $4)) with lines lc rgb variable;
    cd pwd;

    pwd = GPVAL_PWD;
    cd data_folder_2;
    plot for [data in data_files_2] data using (column(5)):1:(rgb($2, $3, $4)) with lines lc rgb variable;
    cd pwd;
} else {
