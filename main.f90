program main
    ...
    integer,        parameter   :: num_layers         = 30!120!30!150
    ...
    real*8                      :: well_2_stop_depth  = 0d0! -0.1d0! 0d0

    real*8                      :: alpha = 0.3!0.2
    
    real*8                      :: poisson_threshold = 0.01
    ...
    integer,        parameter   :: num_k_length       = 40!120!40! 256
    integer,        parameter   :: num_energy         = 40!120!40 !256
    ...
    real*8                      :: width, depth, epsilon_0
    
    ! for poisson distribtuion
    real*8,         allocatable :: density_array(:), potential_array(:), potential_array_new(:), &
                                   bulk_density_array(:), residual_density_array(:), v_part(:), d_part(:)
                                   
    logical                     :: threshold_achieved, condition_satisfied
    
    integer                     :: loop_completed = 0 
    ...
    do i = 1, num_variation
        ...
        if (i == 1) then 
            allocate(residual_density_array( num_layers ))
            allocate(potential_array_new( num_layers ))
            allocate(v_part( num_layers ))
            allocate(d_part( num_layers ))
            residual_density_array( num_layers ) = 0d0
            potential_array_new = 0d0
            v_part = 0d0
            d_part =0d0
        end if 
                         
        do while ( .not. (threshold_achieved))
  
            if ((loop_completed > 0) .and. i > 1 ) then     
                               
                pot(i,:) = potential_array_new

            end if 

            energy_min         = energy_min_p
            energy_max         = energy_max_p
            ...
            ! Determine Maximum number of bands
            do l = 2, 2
                if (j == 1) energy_min = hqtlib_find_energy_min(hr, hrw, pot(l, :), broadening)
                if (k == 1) energy_max = hqtlib_find_energy_max(hr, hrw, pot(l, :), length_scale, broadening)
                max_num_states(l) = hqtlib_find_max_num_states(hr, hrw, pot(l, :), energy_min, energy_max)
                if (max_num_states(l) == 0) then
                    max_num_states(l) = 3        !######
                end if
                if (max_state .ne. -1 .and. max_num_states(l) > max_state) then
                    max_num_states(l) = max_state
                end if
                energy_range(l, :) = route_range_double(energy_min, energy_max, num_energy) - cbm
            end do
            ...
            if(allocated(heatmap_cpu)) then
                deallocate(heatmap_cpu)
                allocate(heatmap_cpu(        size(kp),   max_band - min_band + 1, max_state - min_state + 1, num_energy))
            else 
                allocate(heatmap_cpu(        size(kp),   max_band - min_band + 1, max_state - min_state + 1, num_energy))
            end if 
            if (allocated(den_cpu)) then 
                deallocate(den_cpu)
                allocate(den_cpu(            num_layers, max_band - min_band + 1, max_state - min_state + 1))
            else 
                allocate(den_cpu(            num_layers, max_band - min_band + 1, max_state - min_state + 1))
            end if 
            if (allocated(energymap_cpu)) then 
                deallocate(energymap_cpu)
                allocate(energymap_cpu(      size(kp),   max_state - min_state + 1))
            else 
                allocate(energymap_cpu(      size(kp),   max_state - min_state + 1))
            end if 
            if(allocated(contribution_cpu)) then
                deallocate(contribution_cpu)
                allocate(contribution_cpu(   size(kp),   max_band - min_band + 1, max_state - min_state + 1))
            else 
                allocate(contribution_cpu(   size(kp),   max_band - min_band + 1, max_state - min_state + 1))
            end if 
            if (cpu_is_master()) then
                if(allocated(heatmap)) then 
                    deallocate(heatmap)
                    allocate(heatmap(        size(kp),   max_band - min_band + 1, max_state - min_state + 1, num_energy))
                else 
                    allocate(heatmap(        size(kp),   max_band - min_band + 1, max_state - min_state + 1, num_energy))
                ...
                if (allocated(energymap)) then
                    deallocate(energymap)
                    allocate(energymap(      size(kp),   max_state - min_state + 1))
                else 
                    allocate(energymap(      size(kp),   max_state - min_state + 1))
                end if 
            ...
            do k = k_start, k_stop
                call hqtlib_find_energy_and_weight(energy, weight, num_found, kx(k), ky(k), hr, hrw, pot(i, :) &
                , 1, max_num_states(i))
            ...
            if (cpu_is_master()) then
                ...
                if ((i == 1) .or. ( (i == 2) .and. (loop_completed == 0))) then 
                    call graph_basic_plot("potential", "potential", 1, "Layer", "Potential [eV]", 1, trim(variation_dir))
                else 
                    call graph_basic_plot_two(trim(path)//"Variation 2"//"/"//"potential.dat", &
                    trim(variation_dir)//"potential.dat", trim(variation_dir)//"potential" , &
                    1, "Layer", "Potential [eV]", 1, trim(variation_dir))
                    
                    call graph_basic_plot_two_zoom(trim(path)//"Variation 2"//"/"//"potential.dat", &
                    trim(variation_dir)//"potential.dat", trim(variation_dir)//"potential_zoom" , &
                    1, "Layer", "Potential [eV]", 1, trim(variation_dir))
                ...
                ! store bulk density 
                if (i == 1) then 
                    bulk_density_array = sum(sum(den, 2), 2) * 1d-14
                end if 
                ...
                if (i > 1) then 
                    potential_array_new = 0d0
                    residual_density_array = 0d0
                    v_part = 0d0
                    d_part = 0d0

                    density_array = sum(sum(den, 2), 2)* 1d-14
                    
                    
                    do k = 1, size(density_array)
                        density_array(k) = (density_array(k) - bulk_density_array(k)) !* 1d18 
                    end do 
                                        
                    potential_array = pot(i, :) 
                               
                    ! ε0 = 8.8541878128(13)×10−12 F⋅m−1 (farads per meter), with a relative uncertainty of 1.5×10−10
                    do k = 2, well_2_stop -1 
                   
                    
                        v_part(k) = ( potential_array(k-1)  + (-2) * potential_array(k) +potential_array(k+1) ) &
                                                    *(1.6 * 1d-19)* (1/ (crystal_length **2)) * (1/ (1.6* 1d-19) ) 
                        d_part(k) = density_array(k) * 1d18 * (1/crystal_length) * (1.6 * 1d-19) * &
                                                    (1/( 300 *8.854187812813 * 1d-12 )) * -1

                        residual_density_array(k) =   v_part(k) - d_part(k)
                                                    
                        print*, '__________________________________________________________________________'
                        print*, v_part(k)
                        print*, d_part(k)
                        print*, abs(residual_density_array(k))* (1/( min(abs(v_part(k)), abs(d_part(k))) ))
                        print*, '__________________________________________________________________________'

                    end do


                    potential_array_new( well_2_start  ) =  well_2_start_depth
                    potential_array_new( well_2_stop  ) = 0d0
                    potential_array_new( well_2_stop +1 : size(potential_array_new) )= 0d0

                    ! calculate V_new
                    do k = 2, well_2_stop -1 
                        potential_array_new(k) = potential_array(k) + alpha &
                                                * residual_density_array(k) * (crystal_length **2)
                    end do
                                    
                    condition_satisfied = .true. 
                    do k = 2,  well_2_stop -1 
                        if (abs(residual_density_array(k))* (1/( min(abs(v_part(k)), abs(d_part(k))) )) > poisson_threshold) then 
                ...
                if ((i == 1) .or. ( (i == 2) .and. (loop_completed == 0))) then 
                    ...
                    call graph_colour_two(data_folder_1 = trim(path)//"Variation 2"//"/"//"density.dat", &
                ...
                do j = 1, max_band - min_band + 1
                    ...
                    call graph_basic_plot("density", "density", 1, "Layer", "Carrier Density [cm^{-2}]", 1, trim(band_dir))
                ...
                do j = 1, max_num_states(i) - min_state + 1
                    ...
                    do n = 1, max_band - min_band + 1
                        ...
                        call graph_basic_plot("density", "density", &
                            1, "Layer", "Carrier Density [cm^{-2}]", 1, trim(band_dir))
                        call graph_heatmap_plot("band structure", "band structure", trim(variation_dir)//"meta", trim(band_dir))
                    end do
                    ! Energy Map
                ...
                call export_vstack(trim(path)//"density.dat", reshape((/ &
                    ...
                    sum(den(well_1_start:well_1_stop, :, :)) + &
                        sum(den(well_2_start:well_2_stop, :, :)) / &
                        (abs(well_1_stop + well_2_stop - well_1_start - well_2_start) + 2) /), (/ 1, 8 /)))
                !!!
                !print*, energymap
                ...
                if (i > 2) then
                    call graph_basic_plot("density", "Total_Density", &
                        1, x_label, "Carrier Density [cm^{-2}]", x_offset, trim(path))
                    call graph_basic_plot("density", "reservoir_density", &
                        2, x_label, "Carrier Density [cm^{-2}]", x_offset, trim(path))
                    call graph_basic_plot("density", "transport_density", &
                        3, x_label, "Carrier Density [cm^{-2}]", x_offset, trim(path))
                    call graph_basic_plot("density", "reservoir_plus_transport_density", &
                        4, x_label, "Carrier Density [cm^{-2}]", x_offset, trim(path))
                    call graph_basic_plot("density", "Average_Total_Density", &
                        5, x_label, "Carrier Density [cm^{-2}]", x_offset, trim(path))
                    call graph_basic_plot("density", "Average_reservoir_density", &
                        6, x_label, "Carrier Density [cm^{-2}]", x_offset, trim(path))
                    call graph_basic_plot("density", "Average_transport_density", &
                        7, x_label, "Carrier Density [cm^{-2}]", x_offset, trim(path))
                    call graph_basic_plot("density", "Average_reservoir_plus_transport_density", &
                        8, x_label, "Carrier Density [cm^{-2}]", x_offset, trim(path))
            ...
            if (i > 1) then 
                call cpu_broadcast(potential_array_new, size(potential_array_new)) 
            end if 
            
            call cpu_broadcast(threshold_achieved, 1) 
