program main
    ...
    integer                     :: min_layer_in       = -1
    integer                     :: max_layer_in       = -1
    integer                     :: min_band_in        = -1
    integer                     :: max_band_in        = -1
    integer                     :: min_state_in       = -1
    integer                     :: max_state_in       = -1
    real*8                      :: energy_min_in      = -1d0
    real*8                      :: energy_max_in      = 0d0
    ...
    integer                     :: i, j, k, l, m, e, n, z, c, start_time 
    ...
    do c = 30, 1, -1
        well_start_depth = -2d-3 * c
        
        ! Determine Conductiong Band Minimum
        cbm = hqtlib_find_energy_min(hr, hrw, num_layers)
        
        ! Parameters
        energy_min = energy_min_in + cbm
        energy_max = energy_max_in + cbm
        max_layer  = max_layer_in
        min_layer  = min_layer_in
        max_band   = max_band_in
        min_band   = min_band_in
        max_state  = max_state_in
        min_state  = min_state_in
        
        ! Determine Maximum number of bands
        if (energy_min_in == -1) energy_min = hqtlib_find_energy_min(hr, hrw, pot, broadening)
        if (energy_max_in == -1) energy_max = hqtlib_find_energy_max(hr, hrw, pot, length_scale, broadening)
        ...
        if (max_state_in .ne. -1 .and. max_num_states > max_state_in) then
            max_num_states = max_state
        end if
        
        ! Determine bounds
        if (max_layer_in == -1) max_layer = num_layers
        if (min_layer_in == -1) min_layer = 1
        if (max_band_in == -1)  max_band  = num_bands
        if (min_band_in == -1)  min_band  = 1
        if (max_state_in == -1) max_state = max_num_states
        if (min_state_in == -1) min_state = 1
            ...
            do k = k_start, k_stop
                if (max_num_states .ne. 0)  then
                    ...
                    if (kl(k) .ne. -1) then ! True if on Path
                        ...
                        ! Band Contribution
                        contribution_cpu(kl(k), :, :) = sum(hqtlib_find_band_contribution(energy(:num_found), &
                                                                                          weight(:, :num_found), &
                                                                                          num_bands, &
                                                                min_layer, max_layer, min_band, max_band, min_state, max_state), 1)
                    end if
            ...
            if (cpu_is_master()) then
                ...
                if (r_max <= r_sensitivity .and. reduced_graphs) then
                    do j = 1, max_state - min_state + 1
                        ...
                        call graph_heatmap_plot("band structure", "band structure", &
                            trim(variation_dir)//"meta", trim(energy_state_dir))
                        
                        ! Variation Energy State Energymap
                        call graph_colour_MGX(data_folder    = trim(energymap_dir)//"Energy_level_" &
                                                                   //export_to_string(j, "0")//".dat", &
