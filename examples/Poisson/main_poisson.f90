program main
    ...
    real*8                      :: well_start_depth   = -0.3d0!-2d0 !-0.5d0
    ...
    real*8                      :: alpha              = 0.2d0
    real*8, parameter           :: relative_permativity = 1 !330
    real*8                      :: permativity        = relative_permativity * 8.85418782d-12
    real*8                      :: r_sensitivity      = 1d-6
    ...
    do while (r_max > r_sensitivity)
        ...
        if (cpu_is_master()) then
            ...
            den = 1d18 * den
            do z = 2, well_stop - 1
                r = -1d0 * 1.602d-19**2 * (sum(den(z, :, :)) - density_cbm(z)) / (crystal_length * permativity * 1.602d-19) &
